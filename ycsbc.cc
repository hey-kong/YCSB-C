...
int main(const int argc, const char* argv[]) {
  ...
  if (run) {
    ...
    if (temp_cnt[ycsbc::INSERT])
      printf(
          "insert ops:%7lu  use time:%7.3f s  IOPS:%7.2f iops (%.2f μs/op)\n",
    ...
    if (temp_cnt[ycsbc::READ])
      printf(
          "read ops  :%7lu  use time:%7.3f s  IOPS:%7.2f iops (%.2f μs/op)\n",
    ...
    if (temp_cnt[ycsbc::UPDATE])
      printf(
          "update ops:%7lu  use time:%7.3f s  IOPS:%7.2f iops (%.2f μs/op)\n",
    ...
    if (temp_cnt[ycsbc::SCAN])
      printf(
          "scan ops  :%7lu  use time:%7.3f s  IOPS:%7.2f iops (%.2f μs/op)\n",
    ...
    if (temp_cnt[ycsbc::READMODIFYWRITE])
      printf(
          "rmw ops   :%7lu  use time:%7.3f s  IOPS:%7.2f iops (%.2f μs/op)\n",
          temp_cnt[ycsbc::READMODIFYWRITE],
          1.0 * temp_time[ycsbc::READMODIFYWRITE] * 1e-6,
          1.0 * temp_cnt[ycsbc::READMODIFYWRITE] * 1e6 /
              temp_time[ycsbc::READMODIFYWRITE],
          1.0 * temp_time[ycsbc::READMODIFYWRITE] /
              temp_cnt[ycsbc::READMODIFYWRITE]);
    printf("********************************\n\n");
