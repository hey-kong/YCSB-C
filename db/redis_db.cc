...
int RedisDB::Read(const string& table, const string& key,
  ...
  if (fields) {
    ...
    assert(i == argc - 1);
    redisReply* reply =
        (redisReply*)redisCommandArgv(redis_.context(), argc, argv, argvlen);
    ...
    redisReply* reply =
        (redisReply*)redisCommand(redis_.context(), "HGETALL %s", key.c_str());
    if (!reply) return DB::kOK;
    assert(reply->type == REDIS_REPLY_ARRAY);
    for (size_t i = 0; i < reply->elements / 2; ++i) {
      result.push_back(make_pair(string(reply->element[2 * i]->str),
